...
test.describe('5. DesignPilot', () => {
  ...
  test('Form Submission Test', { tag: '@e2e' }, async ({ page }) => {
    ...
    // Click the "4: LoFi Design" button
    {
    const btnEl = page.getByRole('button', { name: /4: LoFi Design/i }).or(page.getByText('4: LoFi Design', { exact: true })).first();
  ...
  test('Negative: Direct access without login (protected page)', { tag: '@e2e' }, async ({ page }) => {
    // TODO: Without logging in, navigate directly to "/mockup-generator/design/Computers"
