...
test.describe('2. DesignPilot', () => {
  ...
  test('Form Submission Test', { tag: '@e2e' }, async ({ page }) => {
    ...
    // Fill the "Search projects" input field with "test search query"
    await expect(page.getByRole('searchbox').or(page.getByPlaceholder(/Search projects/i)).first()).toBeVisible({ timeout: 10000 });
    await page.getByRole('searchbox').or(page.getByPlaceholder(/Search projects/i)).first().fill('test search query');
    // Click the "Create New Project" button
    {
    const btnEl = page.getByRole('button', { name: /Create New Project/i }).or(page.getByText('Create New Project', { exact: true })).first();
  ...
  test('Negative: Direct access without login (protected page)', { tag: '@e2e' }, async ({ page }) => {
    // TODO: Without logging in, navigate directly to "/mockup-generator"
