...
test.describe('3. DesignPilot', () => {
  ...
  test('Form Submission Test', { tag: '@e2e' }, async ({ page }) => {
    ...
    // Fill the "Website URL to scan" input field with "test value"
    await expect(page.getByLabel(/Website URL to scan/i).or(page.getByPlaceholder(/Website URL to scan/i)).or(page.getByRole('textbox', { name: /Website URL to scan/i })).first()).toBeVisible({ timeout: 10000 });
    await page.getByLabel(/Website URL to scan/i).or(page.getByPlaceholder(/Website URL to scan/i)).or(page.getByRole('textbox', { name: /Website URL to scan/i })).first().fill('test value');
  ...
  test('Button Interactions', { tag: '@e2e' }, async ({ page }) => {
    ...
    // Click the "Collapse menu" button (if visible)
    {
    const btnEl = page.getByRole('button', { name: /Collapse menu/i }).or(page.getByText('Collapse menu', { exact: true })).first();
    if (await btnEl.isVisible({ timeout: 5000 }).catch(() => false)) {
    await btnEl.click();
    }
    }
  ...
  test('Negative: Direct access without login (protected page)', { tag: '@e2e' }, async ({ page }) => {
    // TODO: Without logging in, navigate directly to "/design-audit"
