...
for (let e2eIdx = 0; e2eIdx < E2E_ACCOUNTS.length; e2eIdx++) {
  ...
  test.describe(`2. DoorDash Payroll Portal — ${e2eAccount.label}`, () => {
    ...
    test('Button Interactions', { tag: '@e2e' }, async ({ page }) => {
        ...
        // Click the "PayslipsEmployee Section" button (if visible)
        {
        const btnEl = page.getByRole('button', { name: /PayslipsEmployee Section/i }).or(page.getByText('PayslipsEmployee Section', { exact: true })).first();
        if (await btnEl.isVisible({ timeout: 5000 }).catch(() => false)) {
        await btnEl.click();
        }
        }
    ...
    test('Interactive Elements Test', { tag: '@e2e' }, async ({ page }) => {
        ...
        // Click the "PayslipsEmployee Section" button
        {
        const btnEl = page.getByRole('button', { name: /PayslipsEmployee Section/i }).or(page.getByText('PayslipsEmployee Section', { exact: true })).first();
        if (await btnEl.isVisible({ timeout: 5000 }).catch(() => false)) {
        await btnEl.click();
    ...
    test('Negative: Direct access without login (protected page)', { tag: '@e2e' }, async ({ page }) => {
        // TODO: Without logging in, navigate directly to "/payslips"
