...
for (let e2eIdx = 0; e2eIdx < E2E_ACCOUNTS.length; e2eIdx++) {
  ...
  test.describe(`5. DoorDash Payroll Portal — ${e2eAccount.label}`, () => {
    ...
    test('Form Submission Test', { tag: '@e2e' }, async ({ page }) => {
        ...
        await page.goto('/settings');
        await page.waitForLoadState('domcontentloaded');
        // Fill the "e.g., Spouse, Parent" input field with "test value"
        await expect(page.getByLabel(/e\.g\., Spouse, Parent/i).or(page.getByPlaceholder(/e\.g\., Spouse, Parent/i)).or(page.getByRole('textbox', { name: /e\.g\., Spouse, Parent/i })).first()).toBeVisible({ timeout: 10000 });
        await page.getByLabel(/e\.g\., Spouse, Parent/i).or(page.getByPlaceholder(/e\.g\., Spouse, Parent/i)).or(page.getByRole('textbox', { name: /e\.g\., Spouse, Parent/i })).first().fill('test value');
        // Click the "Security & Login" button
        {
        const btnEl = page.getByRole('button', { name: /Security & Login/i }).or(page.getByText('Security & Login', { exact: true })).first();
    ...
    test('Interactive Elements Test', { tag: '@e2e' }, async ({ page }) => {
        ...
        // Click the "Tax Documents" button
        {
        const btnEl = page.getByRole('button', { name: /Tax Documents/i }).or(page.getByText('Tax Documents', { exact: true })).first();
        if (await btnEl.isVisible({ timeout: 5000 }).catch(() => false)) {
        await btnEl.click();
    ...
    test('Negative: Direct access without login (protected page)', { tag: '@e2e' }, async ({ page }) => {
        // TODO: Without logging in, navigate directly to "/settings"
